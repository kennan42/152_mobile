...
+ "   where \"TRAVLSGRP\" = '')) ";
  ...
  console.log("aaaaaaaaaaaaaaaaaaaaa:"+sql);
   ...
   + " group by \"TRAVID\",left(\"TRAVLSGRP\",3))";
    ...
    + " order by \"STARTDAT_MIN\" asc"; 
       ...
       sql = sql.replace(":usrId",userId).replace(":yMont_S",startDate).replace(":yMont_E",endDate);  
         jdbc.open(function(err, conn) { 
             if (err != null) {  
                  cb("3:jdbc open ERROR", null);
                 }
            jdbc.executeQuery(sql, function(err, rows) {  
                 if (err != null) { 
                     cb("3:jdbc Query  ERROR", null); 
                   } else{
                       jdbc.close(function(err) {
                            if(err != null){  
                              cb("3:JDBC CLOSE ERROR", null);
                                } 
                        });   
                  var keys = [
                        {
     ...
     function(cb){ 
	  ...
	  + " order by \"TRAVGRPID\",\"STARTDAT\" ";
       ...
       sql = sql.replace(":usrId",userId).replace(":yMont_S",startDate).replace(":yMont_E",endDate);   
         jdbc.open(function(err, conn) { 
             if (err != null) {  
                  cb("4:jdbc open ERROR", null);
                 }
            jdbc.executeQuery(sql, function(err, rows) { 
                 if (err != null) { 
                     cb("4:jdbc Query  ERROR", null); 
                   } else{
                       jdbc.close(function(err) {
                            if(err != null){  
                              cb("4:JDBC CLOSE ERROR", null);
                                } 
                        });   
                   cb(null,rows);  
                   }
